...
public class TrieST<Value> {
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        // print results
        if (st.size() < 100) {
        	lib.StdOut.println("keys(\"\"):");
            for (String key : st.keys()) {
            	lib. StdOut.println(key + " " + st.get(key));
            }
            lib.StdOut.println();
        }

        lib.StdOut.println("longestPrefixOf(\"shellsort\"):");
        lib.StdOut.println(st.longestPrefixOf("shellsort"));
        lib.StdOut.println();

        lib.StdOut.println("keysWithPrefix(\"shor\"):");
        for (String s : st.keysWithPrefix("shor"))
        	lib.StdOut.println(s);
        lib.StdOut.println();

        lib.StdOut.println("keysThatMatch(\"sea\"):");
